...
if [ "$INSTALL_DIR" = "/usr/local/bin" ] && [ "$(id -u)" -ne 0 ]; then
    ...
    echo "Please run: curl -sSL https://raw.githubusercontent.com/skbasava/sk-shell/main/install.sh | sudo sh"
    exit 1
fi

# Download the script
curl -sSL https://raw.githubusercontent.com/skbasava/sk-shell/main/sk.sh -o "$INSTALL_DIR/$SCRIPT_NAME"
